...
func (p FileSnapshotProvider) Load()(AccountSnapshot,error){
	raw,err:=os.ReadFile(p.Path);if err!=nil{return AccountSnapshot{},err}
	snap,err:=decodeFlexibleSnapshot(raw,p.Name())
	if err==nil{if info,e:=os.Stat(p.Path);e==nil && snap.CapturedAt.IsZero(){snap.CapturedAt=info.ModTime()}}
	return snap,err
}
...
func (p ToolboxProvider) Load()(AccountSnapshot,error){
	profile:=strings.TrimSpace(p.Profile);if profile==""{return AccountSnapshot{},errors.New("toolbox profile is required")}
	client:=p.Client;if client==nil{client=&http.Client{Timeout:15*time.Second}}
	endpoint:=toolboxProfilesURL+"?profile="+url.QueryEscape(profile)
	req,err:=http.NewRequest(http.MethodGet,endpoint,nil);if err!=nil{return AccountSnapshot{},err}
	req.Header.Set("Accept","application/json")
	resp,err:=client.Do(req);if err!=nil{return AccountSnapshot{},fmt.Errorf("toolbox profile request: %w",err)}
	defer resp.Body.Close()
	if resp.StatusCode==http.StatusNotFound{return AccountSnapshot{},fmt.Errorf("Toolbox public profile %q was not found; make the profile public and use its main character name",profile)}
	if resp.StatusCode!=http.StatusOK{return AccountSnapshot{},fmt.Errorf("Toolbox profile returned HTTP %d",resp.StatusCode)}
	raw,err:=io.ReadAll(io.LimitReader(resp.Body,24<<20));if err!=nil{return AccountSnapshot{},err}
	snap,err:=decodeFlexibleSnapshot(raw,p.Name());if err!=nil{return AccountSnapshot{},fmt.Errorf("Toolbox profile parse: %w",err)}
	if snap.AccountName==""{snap.AccountName=profile}
	return snap,nil
}
...
func decodeFlexibleSnapshot(raw []byte,source string)(AccountSnapshot,error){
	...
	dec:=json.NewDecoder(strings.NewReader(string(raw)));dec.UseNumber()
	if err:=dec.Decode(&generic);err!=nil{return AccountSnapshot{},fmt.Errorf("invalid JSON: %w",err)}
	generic=normalizeJSONNumbers(generic).(map[string]any)
	unwrapped,names,schema,warnings:=unwrapCommunityPayload(generic)

	snap:=normalizeGenericSnapshot(unwrapped,source)
	snap.Schema=schema
	snap.Warnings=append(snap.Warnings,warnings...)
	snap.Raw=unwrapped

	if len(snap.Characters)==0 { snap.Characters=extractRawCharacters(unwrapped,names) }
	if snap.AccountName=="" && len(names)>0 { snap.AccountName=names[0] }

	rawSystems,detected:=detectRawSystems(unwrapped)
	if snap.Systems==nil{snap.Systems=map[string]SystemState{}}
	for id,state:=range rawSystems{
		old,ok:=snap.Systems[id]
		if !ok { snap.Systems[id]=state; continue }
		if old.Evidence=="" { old.Evidence=state.Evidence }
		if old.Confidence==0 { old.Confidence=.55 }
		snap.Systems[id]=old
	}
	...
	if snap.CapturedAt.IsZero(){snap.CapturedAt=time.Now()}
	if len(snap.Characters)==0 { snap.Warnings=append(snap.Warnings,"No characters could be extracted from this source") }
	if len(snap.Systems)==0 { snap.Warnings=append(snap.Warnings,"No supported IdleOn systems were recognized in this source") }
	return snap,nil
}
...
func normalizeGenericSnapshot(generic map[string]any,source string)AccountSnapshot{
	...
	for id:=range idleonSystems{
		...
		for _,key:=range keys{
			if value,ok:=findNumericDeep(generic,key,6);ok{
				progress:=value
				if progress>1&&progress<=100{progress/=100}
				if progress>1{progress=1};if progress<0{progress=0}
				s.Systems[id]=SystemState{Progress:progress,Ready:true,Evidence:"Imported from "+source+" field "+key,Confidence:.70}
				break
			}
		}
	}
	if chars:=findArrayDeep(generic,[]string{"characters","players","chars"},5);len(chars)>0{
		for idx,rawChar:=range chars{
			...
			if c.Name==""{c.Name=fmt.Sprintf("Character %d",idx+1)}
...
func firstNumber(m map[string]any,keys ...string)(float64,bool){for _,key:=range keys{for k,v:=range m{if !strings.EqualFold(k,key){continue};switch t:=deepParseJSONish(v).(type){case float64:return t,true;case json.Number:n,err:=t.Float64();if err==nil{return n,true}}}};return 0,false}
func findNumericDeep(value any,wanted string,depth int)(float64,bool){if depth<0{return 0,false};value=deepParseJSONish(value);switch t:=value.(type){case map[string]any:for k,v:=range t{if strings.EqualFold(k,wanted){if n,ok:=v.(float64);ok{return n,true};if sub,ok:=v.(map[string]any);ok{if n,ok:=firstNumber(sub,"progress","score","percent","completion","level");ok{return n,true}}}};for _,v:=range t{if n,ok:=findNumericDeep(v,wanted,depth-1);ok{return n,true}};case []any:for _,v:=range t{if n,ok:=findNumericDeep(v,wanted,depth-1);ok{return n,true}}};return 0,false}
func findArrayDeep(value any,wanted []string,depth int)[]any{if depth<0{return nil};value=deepParseJSONish(value);switch t:=value.(type){case map[string]any:for k,v:=range t{for _,w:=range wanted{if strings.EqualFold(k,w){if arr,ok:=v.([]any);ok{return arr}}}};for _,v:=range t{if arr:=findArrayDeep(v,wanted,depth-1);len(arr)>0{return arr}};case []any:for _,v:=range t{if arr:=findArrayDeep(v,wanted,depth-1);len(arr)>0{return arr}}};return nil}
