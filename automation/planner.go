...
func dependencyPenalty(rule SystemRule,snapshot AccountSnapshot)(float64,[]string){penalty:=1.0;var blockers []string;for _,dep:=range rule.Dependencies{state,ok:=snapshot.Systems[dep];if !ok{continue};if state.Progress<.35{penalty*=.72;blockers=append(blockers,dep)}else if state.Progress<.55{penalty*=.9;blockers=append(blockers,dep)}};return penalty,blockers}
...
func scoreSystem(id string,state SystemState,world int,snapshot AccountSnapshot,cfg Config)(Recommendation,bool){
	...
	confidence:=.65;if state.Evidence!=""||state.Current>0||state.Target>0{confidence=.9}else if state.Progress>0{confidence=.8};if len(blockers)>0{confidence*=.9}
	reason:=fmt.Sprintf("%s is roughly %.0f%% complete against the supplied target state.",rule.Label,progress*100);if state.Evidence!=""{reason=state.Evidence};if len(blockers)>0{reason+=fmt.Sprintf(" Blockers currently include %s.",strings.Join(blockers,", "))}
	return Recommendation{ID:"system:"+id,Category:"system",Title:rule.Label,System:id,Score:math.Round(score*100)/100,Confidence:math.Round(confidence*100)/100,Reason:reason,PreferredClass:preferred,Character:character,Routine:routine,SwitchRoutine:switchRoutine,Action:rule.Action,Automatable:cfg.routineReady(routine),EstimatedMinutes:int(math.Max(1,hours*60)),Dependencies:rule.Dependencies},true
}
...
func buildAssessmentWithConfig(game GameState,snapshot AccountSnapshot,cfg Config)Assessment{
	world:=snapshot.World;if world<1{world=1};var recs,timers,greens []Recommendation;progressTotal:=0.0;progressCount:=0
	...
	quick:=[]Recommendation{};for _,r:=range recs{if r.Score>=25&&(r.Category=="timer"||r.EstimatedMinutes<=15){quick=append(quick,r);if len(quick)>=8{break}}};allRecs:=append([]Recommendation(nil),recs...);if len(recs)>15{recs=recs[:15]};if len(greens)>10{greens=greens[:10]}
	health:=0;if progressCount>0{health=int(math.Round(progressTotal/float64(progressCount)*100))};unlocked:=unlockedRuleCount(world);coverage:=0;if unlocked>0{coverage=int(math.Round(float64(progressCount)/float64(unlocked)*100));if coverage>100{coverage=100}}
	age:=int64(0);warnings:=[]string{};if !snapshot.CapturedAt.IsZero(){age=int64(time.Since(snapshot.CapturedAt).Seconds());if age<0{age=0};if age>int64(cfg.Settings.MaxSnapshotAgeSec){warnings=append(warnings,fmt.Sprintf("Account snapshot is %d seconds old; execution should wait for a fresh capture.",age))}};if coverage<35{warnings=append(warnings,"Account-data coverage is limited; recommendations are lower confidence until more systems are parsed.")}
...
func chooseAutomation(assessment Assessment)*Recommendation{if !assessment.Game.Running||assessment.SnapshotAgeSec>600{return nil};for i:=range assessment.Top{r:=&assessment.Top[i];if r.Automatable&&r.Routine!=""&&r.Confidence>=.6{return r}};return nil}
