...
func loadConfig(path string) (Config, error) {
	...
	cfg := defaultConfig()
	if err := json.Unmarshal(raw, &cfg); err != nil { return Config{}, err }
	if cfg.Calibration == nil { cfg.Calibration = map[string]Point{} }
	if cfg.Routines == nil { cfg.Routines = map[string]Routine{} }
	if cfg.SystemRoutines == nil { cfg.SystemRoutines = map[string]string{} }
	if cfg.ClassSwitch == nil { cfg.ClassSwitch = map[string]string{} }
	if strings.TrimSpace(cfg.WindowTitle) == "" { return Config{}, errors.New("windowTitle is required") }
	if strings.TrimSpace(cfg.EmergencyStopKey) == "" { cfg.EmergencyStopKey = "F12" }
	if strings.TrimSpace(cfg.CapturePointKey) == "" { cfg.CapturePointKey = "F8" }
	if cfg.Settings.AssessIntervalSec < 5 { cfg.Settings.AssessIntervalSec = 60 }
	if cfg.Settings.MaxSnapshotAgeSec < 30 { cfg.Settings.MaxSnapshotAgeSec = 600 }
	if cfg.Settings.DefaultCooldownMinutes < 1 { cfg.Settings.DefaultCooldownMinutes = 15 }
	if cfg.Settings.MaxActionsPerHour < 1 { cfg.Settings.MaxActionsPerHour = 12 }
	return cfg, nil
}
...
func (c Config) resolvePoint(step Step, game GameState) (Point, error) {
	if step.Point != "" {
		...
		if !ok { return Point{}, fmt.Errorf("missing calibration point %q", step.Point) }
		if p.NX > 0 || p.NY > 0 {
			if game.Width <= 0 || game.Height <= 0 { return Point{}, errors.New("game window rectangle unavailable for relative calibration") }
			return Point{X: game.X + int(p.NX*float64(game.Width)), Y: game.Y + int(p.NY*float64(game.Height)), NX: p.NX, NY: p.NY}, nil
		}
		return p, nil
	}
	if step.X == 0 && step.Y == 0 { return Point{}, errors.New("click step needs point or x/y") }
...
func (c Config) requiredCalibrationPoints() []string {
	...
	for _, routine := range c.Routines {
		for _, step := range routine.Steps {
			if step.Point != "" && !seen[step.Point] { seen[step.Point] = true; out = append(out, step.Point) }
		}
	}
	return out
}

func (c Config) routineForSystem(system, fallback string) string {
	if v := strings.TrimSpace(c.SystemRoutines[strings.ToLower(system)]); v != "" { return v }
	if fallback != "" {
		if _, ok := c.Routines[fallback]; ok { return fallback }
	}
	return ""
}

func (c Config) switchRoutineForClass(class string) string {
	wanted := normalizeClass(class)
	for key, routine := range c.ClassSwitch {
		if normalizeClass(key) == wanted { return strings.TrimSpace(routine) }
	}
	return ""
}
